...
class SlideSource:
            ...
            if not global_slide_id in self.opened_slides:
                try:
                    ...
                    filepath = os.path.join(
                        self.data_dir, self.slide_map[global_slide_id]['storage_address'])
                    slide = Slide(OpenSlide(filepath))
                    self.opened_slides[global_slide_id] = ExpiringSlide(
                        slide, None)
                except KeyError:
    ...
    def _get_slide(self, global_slide_id):
        r = requests.get(
            self.mapper_address.format(
                global_slide_id=global_slide_id))
        return r.json()
