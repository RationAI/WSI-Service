...
def main():
    ...
    parser.add_argument(
        "--port", default=default_port, help="Port the WSI-Service listens to"
    )
    parser.add_argument(
        "--debug", dest="debug", action="store_true", help="Use the debug config"
    )
    parser.add_argument(
        "--load-example-data",
    ...
    uvicorn.run(
        "wsi_service.api:api", host="0.0.0.0", port=int(args.port), reload=args.debug
    )
