...
class Slide(BaseSlide):
    ...
    def __init__(self, filepath, slide_id):
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"Could not obtain ome metadata ({ex})"
            )
    ...
    def get_region(self, level, start_x, start_y, size_x, size_y, padding_color):
        ...
        if size_x < 1 or size_y < 1 or start_x < 0 or start_y < 0:
            raise HTTPException(
                status_code=422, detail="Requested image region invalid."
            )
        ...
        for i in range(start_tile_x0, end_tile_xn):
            for j in range(start_tile_y0, end_tile_yn):
                with self.locker:
                    ...
                    if fh.tell() != offset:
                        raise HTTPException(
                            status_code=422, detail="Failed reading to tile offset"
                        )
    ...
    def __get_pixel_size_ome_tif(self):
        ...
        if pixel_unit_x == "nm":
            ...
            raise HTTPException(
                status_code=422, detail=f"Invalid pixel size unit ({pixel_unit_x})"
            )
    ...
    def __manipulate_metadata(self, dim_order, p_size_x, p_size_y, size_x, size_y):
        namespace = self.__get_xml_namespace()
        self.parsed_metadata.find(f"{ namespace }Image").find(
            f"{ namespace }Pixels"
        ).set("DimensionOrder", dim_order)
        self.parsed_metadata.find(f"{ namespace }Image").find(
            f"{ namespace }Pixels"
        ).set("SizeX", str(size_x))
        self.parsed_metadata.find(f"{ namespace }Image").find(
            f"{ namespace }Pixels"
        ).set("SizeY", str(size_y))
        self.parsed_metadata.find(f"{ namespace }Image").find(
            f"{ namespace }Pixels"
        ).set("PhysicalSizeX", str(p_size_x))
        self.parsed_metadata.find(f"{ namespace }Image").find(
            f"{ namespace }Pixels"
        ).set("PhysicalSizeY", str(p_size_y))
